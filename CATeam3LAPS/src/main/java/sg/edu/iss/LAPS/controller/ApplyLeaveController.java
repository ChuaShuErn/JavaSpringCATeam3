...
public class ApplyLeaveController {
	...
	@RequestMapping("/applyleave/submit")
	public String createLeaveApplication(@ModelAttribute("leaveapplication") @Valid LeaveApplied application, BindingResult bindingResult, HttpSession session, Model model) {
		...
		for(PublicHoliday holiday : publicHolidaysList) {
			...
			if(holiday.getHolidayStartDate().after(application.getLeaveStartDate()) 
					&& holiday.getHolidayEndDate().before(application.getLeaveEndDate())) {
		...
		for(PublicHoliday holiday : publicHolidaysList) {
			if(holiday.getHolidayStartDate().before(application.getLeaveStartDate()) && holiday.getHolidayEndDate().after(application.getLeaveStartDate())) {
		...
		for(PublicHoliday holiday : publicHolidaysList) {
			if(holiday.getHolidayStartDate().before(application.getLeaveEndDate()) && holiday.getHolidayEndDate().after(application.getLeaveEndDate())) {
		...
		for(PublicHoliday day : holidaysAffectLeave2) {
			WeekdaysPublicHoliday2 = ChronoUnit.DAYS.between(appliedStartDate.toInstant(), day.getHolidayEndDate().toInstant()) + 1;
		}
		
		
		// compute situation 3:
		for(PublicHoliday day : holidaysAffectLeave3) {
			WeekdaysPublicHoliday3 = ChronoUnit.DAYS.between(day.getHolidayStartDate().toInstant(), appliedEndDate.toInstant()) + 1;
		}
		...
		return "redirect:/"; // jump to current leave page or stay here?
